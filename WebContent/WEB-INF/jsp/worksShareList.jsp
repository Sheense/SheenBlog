  ...
  <div class="container">
    ...
    <!-- 博文预览区 -->
      <c:forEach var="item" items="${requestScope.sharelist}" >
            <h3><a href="${pageContext.request.contextPath}/share/WorksShare?id=${item.id}" target="_blank">${item.title}</a></h3>
      		<figure><a href="${pageContext.request.contextPath}/share/WorksShare?id=${item.id}" target="_blank"><img src="${item.simplepic}" alt="${item.simplepic}"></a></figure>
      		<ul>
        		<p> ${item.introduction}</p>
        		<a title="阅读全文" href="${pageContext.request.contextPath}/share/WorksShare?id=${item.id}" target="_blank" class="readmore">阅读全文&gt;&gt;</a>
      		</ul>
      		<p class="dateview"><span>${item.time_string}</span><span>作者：${item.author}</span><span>浏览量：${item.watchnum}</span></p>
      </c:forEach>
