...
<link rel="shortcut icon" href="${pageContext.request.contextPath}/images/3.ico" />
<!--[if lt IE 9]>
      ...
      <nav>
        <div  class="navigation">
          <ul class="menu">
             ...
             <li><a href="#">技术文章</a>
               <ul>
                <c:forEach var="item" items="${requestScope.alltype}" >
                    <li id="${item.type}" onclick="searchByType(this)"> <a id="${item.type}"  onclick="searchByType(this)" href="#" >${item.type}</a></li>
                </c:forEach>
              </ul>
            </li>
            <li><a href="#">关于我</a>
            ...
            <li><a  onclick="alert('功能暂未开发')" href="javascript:void(0);" >给我留言</a> </li>
          </ul>
        </div>
      </nav>
  ...
  <div class="container">
    ...
    <!-- 博文预览区 -->
      <c:forEach var="item" items="${requestScope.bloglist}" >
            <h3><a href="${pageContext.request.contextPath}/searchAll?id=${item.id}" target="_blank">${item.title}</a></h3>
      		<figure><a href="${pageContext.request.contextPath}/searchAll?id=${item.id}" target="_blank"><img src="${item.pic}" alt="${item.pic}"></a></figure>
      		<ul>
        		<p> ${item.introduction}</p>
        		<a title="阅读全文" href="${pageContext.request.contextPath}/searchAll?id=${item.id}" target="_blank" class="readmore">阅读全文&gt;&gt;</a>
      		</ul>
      		<p class="dateview"><span>${item.time_string}</span><span>作者：${item.author}</span><span>类型：[<a href="#">${item.fromwhere}</a>]</span><span>浏览量：${item.scanNum}</span></p>
      </c:forEach>
